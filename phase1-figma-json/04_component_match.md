...
```
✅ 正确写法（component_ref 是 key,不是 type 的值）:
{ "component_ref": "椭圆按钮_活动", "name": "活动_花朵", "w": 205, "h": 238,
  "overrides": {"文本_时间": "00:22:43"} }

❌ 错误写法:
{ "type": "component_ref", "name": "椭圆按钮_活动" }
```

⚠️ **注意**：进度条不能用 `component_ref`（必须手搓双层 RECT，详见下方"进度条特殊说明"）。
...
引擎要求进度条是"**包装 FRAME + 双层 RECT** 三层结构（`组_进度_XXX` 包 `底板_XXX` + `进度条_XXX`）"。组件库的 `进度条_短` / `进度条_宽` 内部是 FRAME 包装结构，与引擎不兼容。

**当前规则（本项目）**：
- **进度条本体一律手搓双层 RECT**，不用 `component_ref` 引用 `进度条_短` / `进度条_宽`
- 进度条**左右两侧的图标**仍可以用 `component_ref`（如 `图标_金币`、`图标_锁`），匹配不到才手搓
- **三层结构**: 双层 RECT 必须包装在 `组_进度_XXX` FRAME 内,三层 XXX 完全一致
- **方向编码进节点名**(后缀 `_rl` / `_bt` / `_tb`,默认横向左→右不加)；**百分比不写到 Figma**(由引擎运行时控制)
- **进度条 RECT 必须比底板 RECT 小**(内缩,大尺寸 10px / 中尺寸 8px/5px / 小尺寸 5px/3px)
- **底板和进度条 RECT 中心对齐**(中心点重合)
- **文本/图标作为外层容器的兄弟元素**,不放进 `组_进度_XXX` 内
- 详见 00 "进度条建模铁律"
...
> 注: `进度条_` 见 S0 进度条建模铁律,与 `进度_` 配对出现,必须在白名单。

### 规则11.5：复用结构内部命名一致性铁律（v20.6 阶段二前提）

> 本规则保证 S7 阶段二脚本能正确把扁平 JSON 抽取成 Component。命名漂移会让脚本抽不出 Component 或错聚。

**铁律**: 当一组 FRAME 在视觉上是"同一个东西的多个实例"（17 个排行榜行 / 5 个导航项 / 8 个活动按钮等），它们的**内部子节点必须严格使用同一套命名模板**。
...
- 网格项（背包格子 / 关卡卡片）
- 重复按钮（活动按钮组 / 导航项组）
- 任何在视频里出现 ≥2 次的相同结构 FRAME

#### 怎么做到

决定一种 Component 的内部结构时（比如"排行榜行 = 底板 + 排名文字 + 头像框 + 名字 + 分数图标 + 分数文字"），**先在心里固定一套命名模板**：
...
S7 阶段二脚本（`extract_components.py`）用结构指纹 + 子节点命名判断"是不是同一个 Component"。命名漂移会让指纹不匹配,结果：
- 17 个排行榜行被识别成 17 个独立 FRAME（不抽 Component）
- 或者部分聚类失败,出现 `排行榜行` + `排行榜行_2` 两个 Component
- 设计师粘 final.json 进 Figma 后,组件库里出现一堆"看起来一样但是分散"的 Component Set

#### 状态多态例外（v20.6+）

实例之间允许的差异**只能是视觉属性**（fill / corner_radius / opacity / 节点显隐），**不能是命名差异**：

```
✅ 列表项_排名6_当前用户       ❌ 列表项_当前
├── 底板_行 (fill: 绿色)       ├── 底板_当前用户行     ← 名字变了
├── 文本_排名                   ├── 文本_排名
└── 图标_头像框                 └── 图标_当前头像框    ← 名字变了
```

颜色/形状变 → OK（脚本归到不同 Variant）
命名变 → 不 OK（脚本判成不同 Component）
