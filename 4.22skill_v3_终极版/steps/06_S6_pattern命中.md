...
🔴 **S7 必命名 `按钮_<动作>` FRAME 不可退化** (详 SKILL 铁律 16 / 07b §0): S6 判按钮后, S7 写时**不能只输出 TEXT layer**, **不能用 `组_`/`容器_`/其他前缀** —— 引擎只对 `按钮_` 前缀 FRAME 生成 REDNodeButton 触控层。例外: 该动作词已在某 `按钮_xxx` FRAME 的 children 内 (父按钮代触), 按铁律 7 不嵌套, 仅写 `文本_<动作>` TEXT。
...
7. **S6 决议表已展示并经用户确认** 🔴: ccb/多态决议表 inline 输出, 用户回复 "决议 OK" — 见下方 "S6 ccb/多态决议表确认 (唯一停点)"

---

## 🔴 S6 ccb/多态决议表确认 (唯一停点)

> 根因 (2026-05-18 Team Tournament 复盘): ccb/多态判断是全流程最易错且 Claude 自校验不可靠的环节 → 必须用户 review 拦截。
> **这是 S0→S11 唯一的停点**, 其他步骤 (S1-S5, S7-S11) 全部不停, 一气呵成。
...
- `group`: 必来自 S3 表 C, 不漏
- `pattern_hit`: 命中的 memory pattern 名 / `无(手搓)` / `component_ref:xxx`
- `read_evidence`: 命中 ui_pattern 必填 (证明你真 Read 了, 不是凭印象)
- `subclass_or_note`: 命中子类 / 差异点 / 手搓理由
- `conclusion`: 该组团最终结构结论
- `abstract_ccb`: true/false (按 06a 3 标准 复用/动态/独立 任一即抽)
- `variant_axes`: 多态轴 + 枚举, 无多态写 `[]`

可选 (不强制): 同时写一份 `s6_coverage.json` 跟 scene.json 同目录方便回查。**不再跑 Python gate 脚本, 不靠 exit 0**, **用户 review 即 validation**。

### 反 from-exemplar 锚定 (保留, 跟 gate 无关)

> 上下文里有相似项目 scene.json = **风险信号不是捷径**, 禁止 from-exemplar 改增量, 每组团独立从视频+memory 重推。
> 教训: Team Tournament 中部多节点进度条被照搬 Team Battle (无此面板) 结构 → 进度条本体整根丢失。
> 详 memory `feedback_s6_pattern_must_scan_memory.md`

---

⛔ 决议表 inline 输出后**必停**, 等用户回复 "决议 OK" 或修正意见:
- 用户回 "OK" 类回复 → **立即进 S7, 之后 S7→S11 全部一气呵成跑完, 不再停**
- 用户回修正意见 → S6 内改决议表 + 再次 inline 输出确认, 直到用户 OK
- **这是 S0→S11 流程的唯一一次停点**。其他步骤 (S1-S5, S7-S11) 一律不停。
- 你的"我扫过 memory pattern 了"自述**不算** validation; 决议表给用户看, 让用户拦截判断错才算。

---

⛔ **06_S6_pattern命中 完成, 立即停止, 等待用户回复「继续 S7 生成骨架 — 等用户明确回复「决议 OK」或修正后才进 S7」**。
