...
function buildSceneForRed() {
  ...
  // [v20.7+ 2026-05-22] sprite 命名消歧 — 同 (scene, variant) 上下文里同名 sprite 会让 PNG 互相覆盖,
  // 走"找最近差异祖先 + 节点名拼接 + token 去重"算法, 直接改 scene.json 节点 name 字段.
  // 例: 3 个 底板_按钮 (在 按钮_信息 / 按钮_关闭 / 按钮_Activate 里) →
  //     底板_按钮_信息 / 底板_按钮_关闭 / 底板_按钮_Activate
  var dedupStats = disambiguateSpritesInScene(sceneOut);
  if (dedupStats.renamed > 0) {
    log('🏷️ sprite 命名消歧: ' + dedupStats.renamed + ' 个节点改名 (避免 PNG 互相覆盖)');
  }
...
// ─────────────────────────────────────────────────────────
//  Sprite 命名消歧 (v20.7+ 2026-05-22)
//  在 scene.json 构建完成后跑一遍, 把同 (scene, variant) 上下文里重名的 sprite 叶子节点
//  按"最近差异祖先 + 节点名拼接 + token 去重"算法重命名, 让 PNG 文件名全局唯一.
// ─────────────────────────────────────────────────────────

// 按 _ split 成 tokens, 重复的只保留第一次出现, 重新拼回
function _spriteTokenDedup(name) {
...
// 递归收集 sprite 叶子节点 (按 name 前缀判定, 跟 _isSpriteName 同口径)
// 返回 [{ node, chain }, ...], chain 是从外到内的祖先 name 数组 (不含自己)
function _collectSpriteLeaves(layers, chain, out) {
  if (!layers) return;
  for (var i = 0; i < layers.length; i++) {
    var l = layers[i];
    if (!l || !l.name) continue;
    if (_isSpriteName(l.name)) {
      out.push({ node: l, chain: chain.slice() });
    }
    if (l.children && l.children.length) {
      _collectSpriteLeaves(l.children, chain.concat([l.name]), out);
    }
  }
}

// 给 collision 组里某个 item 找最近差异祖先名
// 从深到浅扫 item.chain, 第一个"不出现在其他实例 chain 任何位置"的祖先即可
function _findSpriteDisambiguator(target, group) {
  var ch = target.chain;
  for (var i = ch.length - 1; i >= 0; i--) {
    ...
    for (var j = 0; j < group.length; j++) {
      if (group[j] === target) continue;
      if (group[j].chain.indexOf(cand) >= 0) { collide = true; break; }
    }
    if (!collide) return cand;
  }
  return null;  // 完全同链路, 不消歧 (共享 PNG)
}

// 在某 layers 树 (= 一个 scene 或一个 variant) 里做消歧, 返回改名节点数
function _disambiguateSpritesInLayers(layers) {
  var sprites = [];
  _collectSpriteLeaves(layers, [], sprites);

  // 按原 name 分组
  var groups = {};
  for (var i = 0; i < sprites.length; i++) {
    var nm = sprites[i].node.name;
    if (!groups[nm]) groups[nm] = [];
    groups[nm].push(sprites[i]);
  }
  ...
  for (var name in groups) {
    ...
    var grp = groups[name];
    if (grp.length < 2) continue;  // 没碰撞, skip
    for (var k = 0; k < grp.length; k++) {
      var item = grp[k];
      var disamb = _findSpriteDisambiguator(item, grp);
      if (!disamb) continue;  // 同链路, 共享 PNG
      var combined = item.node.name + '_' + disamb;
      var newName = _spriteTokenDedup(combined);
      if (newName !== item.node.name) {
        // 保留 figma 原名 (用于 exportAllSprites 在 figma 树里搜节点),
        // 把 layer.name 改成消歧后的新名 (= PNG 文件名 = 引擎 lookup_image 的 key)
        if (!item.node._orig_name) item.node._orig_name = item.node.name;
...
// 顶层入口: 遍历 scene.json 所有上下文 (每个 screen + 每个 component variant)
// 对每个上下文独立跑消歧, 返回总改名数
function disambiguateSpritesInScene(scene) {
  var renamed = 0;
  // 每个 screen 一个上下文
  if (scene.screens) {
    for (var i = 0; i < scene.screens.length; i++) {
      renamed += _disambiguateSpritesInLayers(scene.screens[i].layers || []);
    }
  }
  // 每个 component variant 一个上下文
  if (scene.components) {
    for (var c = 0; c < scene.components.length; c++) {
      var comp = scene.components[c];
      if (!comp.variants) continue;
      for (var v = 0; v < comp.variants.length; v++) {
        renamed += _disambiguateSpritesInLayers(comp.variants[v].layers || []);
      }
    }
  }
...
async function _exportLayersDriven(layers, sceneName, variantName, figmaRoot, results) {
  for (var i = 0; i < (layers || []).length; i++) {
    ...
    if (_isSpriteName(l.name)) {
      // v20.7+ 2026-05-22: sprite 命名消歧后, l.name 可能是消歧后新名 (如"底板_按钮_关闭"),
      // 但 figma 树里的节点名还是原始名 (如"底板_按钮"). 用 _orig_name 兜底:
      // - 优先用 _orig_name (原始 figma 名) 在 figma 树里搜
      // - PNG 上传名 layer_name = l.name (消歧新名, 跟 scene.json + 引擎 lookup_image 一致)
      var lookupName = l._orig_name || l.name;
      var node = _findInSubtreeByName(figmaRoot, lookupName);
      if (node) {
        try {
          ...
          results.push({
            ...
            layer_name: l.name,                // 消歧新名 → PNG 文件名
        ...
        console.warn('[exportAllSprites] 在', sceneName, '(variant=' + (variantName||'-') + ') 内未找到图层:', lookupName, '(scene.json name=' + l.name + ')');
      }
    }
